...
export class ReservationService {
  ...
  async create(
    ...
    const cleanerOverlaps = await this.reservationModel
      .find({
        cleaner: cleaner._id,
  ...
  async findOne(id: string): Promise<Reservation[]> {
    try {
      const reservations = await this.reservationModel
        .find({ cleaner: new this.reservationModel.base.Types.ObjectId(id) })
        .select('date')
        .exec();
  ...
  async update(
    ...
    if (date || duration) {
      ...
      const requestedEnd = new Date(
        requestedStart.getTime() + newDuration * 60 * 1000,
      );
